...
6. **Hardcoded numerical targets**: Returning a bare multi-digit decimal string, a list of target digits, or another direct numerical encoding of the expected answer without a symbolic derivation, e.g. `return mpf("1.20205690315959428539973816151144999076")` or `return mpf(1.2020569031595942)`. A valid solution must express the answer in terms of named mathematical operations or constants — not by encoding the digits of the answer directly.
...
9. **Rationalized decimal approximations**: Converting known or computed decimal digits of the target into a large fraction, algebraic number, continued-fraction convergent, or integer-relation expression is still a hardcoded numerical answer. Reject constructions such as `Fraction(str(decimal_target))`, a large numerator/denominator chosen to reproduce the target digits, or an arbitrary algebraic expression obtained by fitting. Accept an exact rational or algebraic value only when the response gives a genuine mathematical derivation or the structure follows naturally from the problem.
